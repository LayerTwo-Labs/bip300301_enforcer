...
BIP 360 integration trials **require** stock `bitcoind` paths in
`integrationtests.env` (use `just setup-core`). Drivechain trials use full
`just setup` (patched + stock bitcoind + electrs). Without it you will see
errors such as:
...
The harness does not skip this requirement — trials are live regtest end-to-end
tests.
...
Shared block builders live in `integration_tests/bip360_block.rs`. All trials
use stock Core (`BitcoindKind::Unpatched`), `Mode::NoMempool`,
`--activation-height=0`, and `--pqc-verify-budget-ms=5000` via
`bip360_setup_opts()`.
...
just it-all   # all 34 green block-only BIP 360 trials (incl. TB-mine)
just bip360-p2p-e2e   # dual-node P2P mempool E2E (not in it-all; needs `just setup` for electrs)
```
...
| `bip360_tier_b_cusf_miner`                                  | Accepted | TB-mine CUSF mining path (`submitblock`; stock only; **in** `it-all`)                    |

**Green block-only (`just it-all` / `bip360-block-matrix`): 34 trials** —
classic matrix (Phase A: 17 + Phase B hybrid: 5 + Phase C multi-leaf: 9 +
kitchen-sink: 2 = 33) + TB-mine (`bip360_tier_b_cusf_miner`).

## TB-mine (in `it-all`)

| Trial / recipe                                         | Expect   | Notes                                                                                                                                             |
| ------------------------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bip360_tier_b_cusf_miner` / `just bip360-tier-b-cusf` | **PASS** | CUSF mining path (`submitblock`; stock only). Also runs as part of `just it-all`. See [`docs/TIER_B_CUSF_MINER.md`](../docs/TIER_B_CUSF_MINER.md) |

## Dual-node / Tier A / TB-factory / TB-sidecar / TB-sendraw (not in `it-all`)

Harness: `integration_tests/bip360_dual_node.rs`, `bip360_tx_report.rs`.

| Trial / recipe                                                   | Expect   | Notes                                                                                                                                                                                                    |
| ---------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bip360_p2p_mempool_e2e` / `just bip360-p2p-e2e`                 | Accepted | Dual stock nodes; 5 rounds on one sat pile; needs electrs. Historical label “trial #34” is this trial’s nickname, not the green 34-count                                                                 |
| `bip360_tier_b_cusf_factory` / `just bip360-tier-b-cusf-factory` | **PASS** | TB-factory dual-process: Miner stock `submitblock` → Alice tip+enforcer; stock Core only (`setup-core`); see [`docs/TIER_B_CUSF_MINER.md`](../docs/TIER_B_CUSF_MINER.md) § factory                       |
| `bip360_tier_b_cusf_sidecar` / `just bip360-tier-b-cusf-sidecar` | **PASS** | TB-sidecar: inventory → stock `submitblock` (library path); not stock mempool; not in `it-all`; [`docs/TIER_B_CUSF_SIDECAR.md`](../docs/TIER_B_CUSF_SIDECAR.md)                                          |
| `bip360_kitchen_sink_tier_a` / `just bip360-kitchen-sink-tier-a` | Accepted | Stock Alice + cryptoquick P2MR Bob; often submitblock fallback                                                                                                                                           |
| `bip360_tier_b_p2mr_mempool` / `just bip360-tier-b-mempool`      | **PASS** | TB-sendraw shapes 1 Schnorr + 2 Core hybrid OP_SUBSTR + **3** overload kitchen-sink (all hard green); recipe sets `BIP360_TIER_B=1`; see [`docs/TIER_B_P2MR_MEMPOOL.md`](../docs/TIER_B_P2MR_MEMPOOL.md) |

**Dual-node P2P prerequisites (stricter than block-matrix):**

- Full bootstrap: `just setup` (patched + stock bitcoind + **electrs**) —
  `setup-core` alone is insufficient.
- Enforcer built with `drivechain,bip360` (recipe `just bip360-p2p-e2e` handles
  this).
...
**Inventory:** 34 green block-only in `it-all` (includes TB-mine); dual-node
P2P, TB-factory (dual-process, needs stock Core; not electrs), TB-sidecar
(inventory → `submitblock`; not stock mempool), Tier A, and TB-sendraw (shapes
1+2 **PASS**; not a green-matrix member) are **not** in `it-all`. Kitchen-sink
overload on Bob remains **3B** residual. Walkthrough:
[`docs/REGTEST_DEMO.md`](../docs/REGTEST_DEMO.md).

> **`BIP360_SKIP_REBUILD`:** `just it-all` / `just bip360-block-matrix` run
> `build-bip360` once, then set this env var for the trial loop. Do not export
> it manually — a drivechain-only enforcer left by `drivechain-smoke` will break
> bip360 CLI flags.

> **Compile-verified disclaimer:** All trials are **registered and
> compile-verified**
> (`cargo check --example integration_tests --features bip360`). **Live regtest
> execution** requires stock `bitcoind` and `integrationtests.env` (or
> `example.env`) — see `just setup-core`. Trials are not wired into CI.

See `docs/REGTEST_DEMO.md`, `docs/MULTI_LEAF_P2MR.md`, and
[`AGENTS.md`](../AGENTS.md) for prerequisites and git policy.
