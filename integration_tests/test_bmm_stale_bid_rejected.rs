...
pub const TEST_NAME: &str = "bmm_stale_bid_rejected";
...
async fn test_bmm_stale_bid_rejected_task(mut post_setup: PostSetup) -> anyhow::Result<()> {
    ...
    // enforcer's BDK wallet funded above -- so the stale bid below is a raw,
    // hand-crafted transaction the enforcer's own bid-tracking never sees.
    ...
    let () = post_setup
        ...
        .command::<String, _, _, _, _>(
            [],
            "generatetoaddress",
            ["101".to_owned(), mining_address.clone()],
        )
        .run_utf8()
        .await
        .map(drop)?;
    ...
    let (_, _, tip_0) = get_tip_info(&mut post_setup).await?;
    ...
    let (prev_bytes_1, height_1, tip_1) = get_tip_info(&mut post_setup).await?;
...
/// A losing BMM bid for one mainchain block must not be confirmable as if it
/// were valid for a later block, even by coincidence of a matching h*.
pub async fn test_bmm_stale_bid_rejected(
    bin_paths: BinPaths,
    ...
    let _test_task: util::AbortOnDrop<()> = tokio::task::spawn({
        async move {
            let res = test_bmm_stale_bid_rejected_task(post_setup).await;
