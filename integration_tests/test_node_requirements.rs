...
pub async fn test_assumeutxo_node(setup: PreSetup) -> anyhow::Result<()> {
    ...
    // height, still in IBD, with no block data below the tip.
    ...
    anyhow::ensure!(
        chain_info["blocks"] == fixture.base_height && chain_info["initialblockdownload"] == true,
        "expected tip {} + IBD after snapshot load, got: {chain_info}",
        fixture.base_height,
    );
    ...
    anyhow::ensure!(
        chain_info["initialblockdownload"] == true,
        "the node must still be in IBD at this point, got: {chain_info}"
    );
