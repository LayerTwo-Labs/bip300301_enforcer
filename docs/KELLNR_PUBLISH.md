...
`crates.denver.space`. The enforcer workspace currently pins `bitcoinpqc` from **git**
...
[`libbitcoinpqc-bindings`](https://github.com/cryptoquick/libbitcoinpqc-bindings) — e.g.
`$LIBBITCOINPQC_BINDINGS` or `../libbitcoinpqc-bindings` next to this workspace.
...
- [ ] `CHANGELOG` / release notes for 0.4.0 (ML-DSA-44, SLH-DSA-SHA2-128s, Schnorr bindings)
- [ ] `SECP256K1_SCHNORR` keygen/sign paths tested (32-byte entropy, 32-byte message hash)
...
5. **Verify** on Kellnr UI: crate `bitcoinpqc` version `0.4.0` appears and downloads.

## Post-publish — update consumers

After Kellnr has `bitcoinpqc` 0.4.0:

1. **`bip300301_enforcer/Cargo.toml`** — switch from git pin to registry (optional but recommended):
...
2. **`bip-0360/ref-impl/rust/Cargo.toml`** — align PQC crate version if the ref-impl
   should consume 0.4.0 from Kellnr.
...
Publishing to Kellnr and tagging the upstream git repo are **manual** steps. This
document does not automate credentials, registry ACLs, or cross-repo version bumps.