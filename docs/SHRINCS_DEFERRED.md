...
**Status:** Not implemented. No local reference implementation or consensus test vectors.

## Scope

SHRINCs (and related XMSS hybrid backup signature paths mentioned in BIP 360 discussions)
are **out of scope** for the current `bip300301_enforcer` prototype.
...
Per [`DESIGN.md`](../../DESIGN.md) prototype limits, a SHRINCs backup signature might
allow up to **30 000 bytes** when annex flag `0x50` is present — that limit is
documented in design notes only; **no code enforces it**.
...
- Overload model (current): [`BIP360_OVERLOAD_ADDENDUM.md`](./BIP360_OVERLOAD_ADDENDUM.md)
