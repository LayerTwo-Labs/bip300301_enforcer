...
# on every push to them. Tested alongside bitcoin-patched so that L1 changes
# which break the enforcer surface here. scripts/fetch_ecash_bitcoin.sh
# downloads and provenance-checks them; CI reads this list via
# `--print-channels`.
...
# repeating the literal. Takes the flavor as an argument.
if [ "${1:-}" = '--print-regtest-magic' ]; then
    ecash_regtest_magic "${2:?--print-regtest-magic needs a flavor}"
    exit 0
fi

# Likewise for an ecash build's OP_DRIVECHAIN opcode.
if [ "${1:-}" = '--print-op-drivechain' ]; then
    ecash_op_drivechain "${2:?--print-op-drivechain needs a flavor}"
    exit 0
fi
...
# build (Rosetta). Stock Bitcoin Core and the ecash channel builds have
# native arm64 darwin builds.
OS="$(uname -s | tr '[:upper:]' '[:lower:]')"
ARCH="$(uname -m)"
case "$OS-$ARCH" in
    linux-x86_64)  STOCK_TARGET="x86_64-linux-gnu";    PATCHED_TARGET="x86_64-unknown-linux-gnu"; ECASH_TARGET="x86_64-unknown-linux-gnu" ;;
    darwin-x86_64) STOCK_TARGET="x86_64-apple-darwin"; PATCHED_TARGET="x86_64-apple-darwin";      ECASH_TARGET="x86_64-apple-darwin" ;;
    darwin-arm64)  STOCK_TARGET="arm64-apple-darwin";  PATCHED_TARGET="x86_64-apple-darwin";      ECASH_TARGET="aarch64-apple-darwin" ;;
...
for channel in $ECASH_CHANNELS; do
    "$REPO_ROOT/scripts/fetch_ecash_bitcoin.sh" "$channel" "$ECASH_TARGET" "$(ecash_dir "$channel")"
done
...
echo "Run integration tests with: just test-it [--bitcoind bitcoin-patched|unpatched|stock-X.Y|alphanet|betanet|all]"
