...
/// Check whether the node's REST server (`-rest`) is enabled and reachable,
/// tolerating the 503s Bitcoin Core serves while warming up. The enforcer
/// batch-fetches block headers over REST during sync when it is available,
/// and falls back to JSON-RPC when it is not. Returns `false` if the REST
/// server is disabled.
pub async fn check_rest_server_available(
    rest_client: &MainRestClient,
) -> Result<bool, RestServerCheckError> {
