...
impl Wallet {
    ...
    /// The output shape shared by every BMM bid transaction, fresh or
    /// replacement: the M8 OP_RETURN as the first output (BIP301), the whole
    /// bid as absolute fee, and the requested locktime.
    fn configure_bmm_builder(
        builder: &mut bdk_wallet::TxBuilder<
            '_,
    ...
    /// Try to build a BIP125 replacement for `tracked`, the previous
    /// in-flight bid for this sidechain. Its transaction may still be in
    /// bitcoind's mempool, so the replacement must reuse its inputs; building
    /// fresh instead would let coin selection pick different inputs and leave
    /// two live bids. A plain fee bump is not a separate case: with unchanged
    /// arguments the rebuilt M8 is byte-identical, so the replacement *is*
    /// the bump.
    ///
    /// The fallback ladder is dictated by where BDK may hold the old bid:
    /// canonical (fee-bump it), evicted by `connect_block` but probably still
    /// in bitcoind's mempool (reuse its now-freed inputs by hand), or truly
    /// spent. Returns `Ok(None)` in the cases where nothing live is left to
    /// replace, and the caller should build a fresh transaction: erroring
    /// instead would strand the sidechain, since a block we did not produce
    /// never clears the tracked row.
    async fn try_build_bmm_replacement(
            ...
            .await
        {
            ...
            // Not canonical to BDK is not the same as gone: the bid is
            // probably still in bitcoind's mempool, evicted from our view
            // by `connect_block`. That eviction frees its inputs, so select
            // them by hand.
            Err(error::BuildBmmFeeBump::BuildFeeBump(
                ...
                {
                    Ok(psbt) => Ok(Some(psbt)),
                    // The inputs really are gone -- spent for real by a
                    // confirmed transaction rather than merely held by an
                    // evicted one. Nothing is left to collide with, so a
                    // fresh build is correct.
                    Err(error::BuildBmmTx::AddUtxo(bdk_wallet::AddUtxoError::UnknownUtxo(
                        outpoint,
                    ))) => {
                        tracing::debug!(
                            old_txid = %tracked.txid,
    ...
    pub async fn create_bmm_request(
        ...
        // unchanged amount, which is treated as an idempotent resubmission
        // (e.g. protects against a gRPC client retrying on a timeout).
        if same_slot
            ...
            match bid_amount.to_sat().cmp(&tracked.fee_sats) {
                ...
                std::cmp::Ordering::Equal => {
                    let tx: bdk_wallet::bitcoin::Transaction =
                        bitcoin::consensus::deserialize(&tracked.raw_tx)
