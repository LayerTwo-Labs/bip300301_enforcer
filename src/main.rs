use std::{fs::OpenOptions, future::Future, net::SocketAddr, path::PathBuf, sync::Mutex};
...
// Configure logger.
fn set_tracing_subscriber(log_file: &PathBuf, log_level: tracing::Level) -> miette::Result<()> {
    let targets_filter = {
